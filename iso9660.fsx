    ...
    member this.UnpackSUSP(maxlen: int) : SUSPEntry option =
        if maxlen < 4 then None
            ...
            if payloadLen < 0 || maxlen < int lenByte then
                ...
                | :? SUSPError ->
                    cursor <- startCursor
                    let rawLen = Math.Max(0, Math.Min(payloadLen, maxlen - 4))
                    let raw = this.UnpackRaw(rawLen)
...
let rec listDir (indent: string) (r: Record) =
    printfn "%s%s" indent r.Name
    if r.IsDirectory then
        for child in r.Children do
            if child.Name <> "." && child.Name <> ".." then
                listDir (indent + "  ") child
...
// let directory = iso.Root.Children |> List.find(fun r->r.IsDirectory && r.Name.Equals("directory"));;
// let readme = iso.Record("README.TXT").Content |> System.Text.Encoding.ASCII.GetString;;
// let grub = directory.Children |> List.find (fun r -> r.IsDirectory && r.Name = "grub");;
// let grubCfg = grub.Children |> List.find (fun r -> not r.IsDirectory && r.Name = "grub.cfg");;
// let text = grubCfg.Content |> System.Text.Encoding.UTF8.GetString;;
// printfn "%s" text;;
// iso.Close()
